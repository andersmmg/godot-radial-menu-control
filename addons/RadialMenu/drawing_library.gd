...
static func calc_ring_segment(
	inner_radius, outer_radius, start_angle, end_angle, offset = Vector2.ZERO
):
	"""
...
static func calc_ring_segment_centers(
	radius, n_points, start_angle, end_angle, offset = Vector2.ZERO, last_included = false
):
	var coords = PackedVector2Array()
	var angle
	if last_included:
...
static func calc_ring_segment_AABB(
	inner, outer, start_angle, end_angle, center = Vector2.ZERO
) -> Rect2:
	"""
...
static func calc_faux_ring_segment(
	inner_radius, outer_radius, separation: float, start_angle, end_angle, offset = Vector2.ZERO
) -> PackedVector2Array:
