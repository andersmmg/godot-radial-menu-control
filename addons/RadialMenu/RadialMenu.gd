...
func _create_subtree() -> void:
	if not get_node_or_null(ITEM_ICONS_NAME):
		...
		add_child(item_icons, false, Node.INTERNAL_MODE_BACK)
	if not get_node_or_null(ITEM_LABEL_NAME):
		var item_label := RichTextLabel.new()
		item_label.name = ITEM_LABEL_NAME
		add_child(item_label, false, Node.INTERNAL_MODE_BACK)
...
func _draw_center() -> void:
	...
	if (
		...
		get_node(ITEM_LABEL_NAME).visible = false
...
func _draw_label() -> void:
	...
	var item_label: RichTextLabel = get_node(ITEM_LABEL_NAME)
	...
	item_label.size = Vector2(center_radius * 2, center_radius)
	item_label.add_theme_color_override("default_color", color)
	item_label.add_theme_font_size_override("bold_italics_font_size", fontsize)
	item_label.add_theme_font_size_override("italics_font_size", fontsize)
	item_label.add_theme_font_size_override("mono_font_size", fontsize)
	item_label.add_theme_font_size_override("normal_font_size", fontsize)
	item_label.add_theme_font_size_override("bold_font_size", fontsize)
	item_label.set_position(Vector2(
			_center_offset.x - (item_label.size.x/2), 
			_center_offset.y - (item_label.size.y/2)
	))
	#var break_flag := TextServer.BREAK_WORD_BOUND && TextServer.BREAK_MANDATORY
	#var size := font.get_multiline_string_size(text, HORIZONTAL_ALIGNMENT_CENTER, center_radius, fontsize, 4, break_flag)
	#
	#if center_radius > size.x / 2.0 and center_radius > size.y / 2.0:
		## show text if it fits inside the center ring
		#var pos := _center_offset - Vector2(size.x / 2.0, (size.y / 2.0) -font.get_ascent())
		#draw_multiline_string(font, pos, text, HORIZONTAL_ALIGNMENT_CENTER, mini(size.x, center_radius), fontsize, 4, color, break_flag)
	#else:
...
func _clear_item_icons() -> void:
	var p = get_node(ITEM_ICONS_NAME)
	if not p:
		return
...
func _create_item_icons() -> void:
	...
	for i in range(n):
		...
		if item != null:
			...
			get_node(ITEM_ICONS_NAME).add_child(sprite)
...
func _clear_items() -> void:
	if not has_node("ItemIcons"):
		return
	var n = get_node(ITEM_ICONS_NAME)
